...
func (pgdb *PostgresDB) AddAllValue(ctx context.Context, metrics []models.Metrics) error {
	...
	for _, v := range metrics {
		...
		if v.MType == "gauge" {
			_, err = tx.ExecContext(ctx,
				`INSERT INTO metrics (ID,MTYPE,VALUE) VALUES ($1, $2, $3) ON DUPLICATE KEY UPDATE ID = $1;`, v.ID, "gauge", v.Value)
			if err != nil {
			...
			_, err = tx.ExecContext(ctx,
				`INSERT INTO metrics (ID,MTYPE,DELTA) VALUES ($1, $2, $3) ON DUPLICATE KEY UPDATE ID = $1;`, v.ID, "counter", v.Delta)
			if err != nil {
				// если ошибка, то откатываем изменения
				tx.Rollback()
				return err
